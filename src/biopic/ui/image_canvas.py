...
from biopic.ui.image_canvas_items import _ProjectionItem, _SelectionHandleItem
...
class ImageCanvas(ImageCanvasSelectionMixin, QGraphicsView):
    ...
    def set_tool_mode(self, mode: str) -> None:
        ...
        elif mode in {"select", "ellipse_select", "free_select", "crop"}:
            self.setDragMode(QGraphicsView.DragMode.NoDrag)
    ...
    def mousePressEvent(self, event: QMouseEvent) -> None:
        ...
        if self._tool_mode in {"select", "ellipse_select", "crop"}:
        ...
        if self._tool_mode == "zoom":
            ...
            self._display_shell.zoom_by(factor)
            return
        if self._tool_mode in _POINT_TOOL_MODES:
            ...
            if 0 <= y < self._pixels.shape[0] and 0 <= x < self._pixels.shape[1]:
                if self._tool_mode in _PAINT_TOOL_MODES:
    ...
    def mouseMoveEvent(self, event: QMouseEvent) -> None:
        ...
        if self._drag_start is not None and self._tool_mode in {"select", "ellipse_select", "crop"}:
            point = self.mapToScene(event.position().toPoint())
        ...
        if (
            self._pixels is not None
            and self._tool_mode in _PAINT_TOOL_MODES
    ...
    def mouseReleaseEvent(self, event: QMouseEvent) -> None:
        ...
        if self._paint_stroke_active and self._tool_mode in _PAINT_TOOL_MODES:
            point = self.mapToScene(event.position().toPoint())
            x_float, y_float = float(point.x()), float(point.y())
            x, y = int(x_float), int(y_float)
            if (
...
def _annotation_path(
    ...
    if kind is AnnotationKind.WEDGE:
        ...
        nx = -dy / length
        ny = dx / length
        half_width = max(5.0, line_width * 4.0)
        path.moveTo(end)
        path.lineTo(start.x() + nx * half_width, start.y() + ny * half_width)
        path.lineTo(start.x() - nx * half_width, start.y() - ny * half_width)
        path.closeSubpath()
        return path
    path.moveTo(start)
